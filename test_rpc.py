"""
🌐 test_rpc.py - Тестирование RPC подключений к блокчейн-сетям

Функциональность:
- 🌐 Тестирование подключений к 20+ блокчейн-сетям
- 🔗 Проверка RPC endpoints на доступность
- 📊 Детальный отчет о статусе каждой сети
- 🔧 Поддержка Alchemy API для приватных RPC
- ⚡ Быстрая диагностика проблем с подключением

Поддерживаемые сети:
- Ethereum, Optimism, BSC, Polygon, Arbitrum
- Avalanche, Fantom, Gnosis, Celo, Core
- Harmony, Klaytn, Moonbeam, Moonriver
- Linea, Base, ZkSync, Polygon zkEVM, Nova

Используемые RPC провайдеры:
- Alchemy API (для приватных ключей)
...
def test_block_number():
    # Используйте публичный RPC или свой API ключ из переменных окружения
    url = 'https://rpc.ankr.com/eth/YOUR_ANKR_API_KEY'  # Замените на свой ключ
...
# Ваш API-ключ от Alchemy (загружается из переменных окружения)
